...
import { Plus, Pencil, Trash2, Check, Search, ChevronsUpDown, ChevronUp, ChevronDown, SlidersHorizontal } from "lucide-react";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        ...
        await apiClient(`/api/services/${editingId}`, { method: "PUT", body: JSON.stringify(payload) });
        onServicesChange(services.map((s) => (s.id === editingId ? { ...s, ...form, isActive: isActiveEdit } : s)));
        onToast("Послугу оновлено", "success");
      } else {
        const created = await apiClient<Service>("/api/services", { method: "POST", body: JSON.stringify(form) });
          ...
          <table className="w-full text-left text-sm text-slate-600">
            <thead className="sticky top-0 z-10 bg-slate-50 text-slate-900 font-semibold border-b border-slate-200">
              <tr>
                ...
                <th className="px-6 py-4 cursor-pointer select-none hover:text-blue-600 transition-colors" onClick={cycleSort}>
            ...
            <tbody className="divide-y divide-slate-100">
              {sorted.map((s) => (
                <tr key={s.id} className="hover:bg-slate-50 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    <button onClick={() => handleToggleStatus(s)} title="Натисніть для зміни статусу" className="cursor-pointer">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <Select
              ...
              onChange={(val) => { setForm({ ...form, specializationId: val }); clearErr("specializationId"); }}
              options={specializations.map((sp) => ({ value: sp.id, label: sp.name }))}
          ...
          <div className="sm:col-span-2 flex justify-end gap-2 pt-2">
            <button type="button" onClick={() => setShowForm(false)} className="px-4 py-2 text-sm font-medium text-slate-600 hover:text-slate-900 transition-colors cursor-pointer">
              Скасувати
            </button>
