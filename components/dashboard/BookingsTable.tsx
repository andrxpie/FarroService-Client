...
interface BookingsTableProps {
  bookings: Booking[];
  onAction: (id: number, status: BookingStatus) => void;
}
...
export const BookingsTable: React.FC<BookingsTableProps> = ({
  ...
  <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
      ...
      <table className="w-full text-left text-sm text-slate-600">
        <thead className="bg-slate-50 text-slate-900 font-semibold border-b border-slate-200">
          <tr>
            ...
            <th className="px-6 py-4">Клієнт / Адреса</th>
          ...
          {bookings.map((b) => {
            ...
            return (
              ...
              >
                ...
                <td className="px-6 py-4 text-slate-900">{service?.title}</td>
                <td className="px-6 py-4">
    ...
    {bookings.length === 0 && (
      <div className="p-12 text-center text-slate-400">Записів не знайдено</div>
    )}
  </div>
);
